...
if kubectl get configmap webapp-deployment-config-map &> /dev/null; then
...
# Check if the Deployment uses the ConfigMap for the APP_NAME environment variable
env_value=$(kubectl get deployment webapp-deployment -o=jsonpath='{.spec.template.spec.containers[0].env[0].valueFrom.configMapKeyRef.name}')
if [ "$env_value" == "webapp-deployment-config-map" ]; then
    echo "Validation PASSED: Deployment webapp-deployment uses the ConfigMap for the APP_NAME environment variable."
else
    echo "Validation FAILED: Deployment webapp-deployment does not use the ConfigMap for the APP_NAME environment variable."
    exit 1
fi

# Check if the pods are running
if kubectl get pods -l app=webapp-deployment | grep Running &> /dev/null; then
